...
class ExperienceReplay:
    ...
    def store(self,data):
        if len(self.header) !=  len(data):
            raise ValueError("wrong number of stored data")
        self.replay_buffer.append(data)

    def replay(self):
        if len(self.replay_buffer)>2:
            print("experience replay {} are stored in buffer ".format(len(self.replay_buffer)))
           ...
           # We sample three action in order to replay them. The more the loss the more the chance for an action to be taken 
            ...
            for idx, row in data.iterrows():
                ...
                with tf.GradientTape(watch_accessed_variables=False) as tape:
                    ...
                    self.replay_buffer[idx][-1] = loss.numpy()
